...
while ($c = mysqli_fetch_array($sql2)) {
    $array[$a] = array();
    $ramal[$a] = array();
    $sql1 = mysqli_query($conn, "SELECT penjualan as dodol FROM peramalan WHERE id_produk = '$c[id_barang]'");

    while ($row = mysqli_fetch_array($sql1)) {
        array_push($array[$a], $row['dodol']); //penjualan
    }

    $query1 = mysqli_query($conn, "SELECT peramalan as ya FROM peramalan WHERE id_produk = '$c[id_barang]'");
    ...
    ?>
                <div class="panel panel-flat">
                    <div class="panel-heading">
                        <h6 class="panel-title text-semibold">Grafik Penjualan <?= $c['nama_barang'] ?></h6>
