...
$sql2 = mysqli_query($conn, "SELECT barang.id_barang,barang.nama_barang FROM barang"); // memanggil data produk
while ($c = mysqli_fetch_array($sql2)) {
$a = 0.1;
$sql = mysqli_query($conn, "SELECT SUM(jumlah_keluar) AS lembar, MONTH(tanggal_keluar) as tahun FROM penjualan WHERE 
 barang_id='$c[id_barang]' GROUP BY MONTH(tanggal_keluar) LIMIT 5"); // memanggil jumlah data penjualan berdasarkan bulan dan tahun
    ...
    for ($loop = 0; $loop < 9; $loop++) {
        ...
        <h2><?php echo $c['nama_barang'] . "($a)" ?></h2>
        <table style="width:50%" border="1">
            <tr>
                <th class="text-center" width="100">Periode (Bulan)</th>
                <th class="text-center" width="100">Xt</th>
                <th class="text-center" width="100">S't</th><!-- Peramalan pertama -->
                <th class="text-center" width="100">S''t</th><!-- Peramalan Kedua -->
                <th class="text-center" width="100">Nilai a</th><!-- konstanta -->
                <th class="text-center" width="100">Nilai b</th><!-- Slope -->
                <th class="text-center" width="100">Ramalan</th><!-- forecast -->
        ...
        foreach ($periode as $key => $data) {
            ...
            mysqli_query($conn, "INSERT INTO peramalan VALUES ('','$c[id_barang]','$tmptggl','$datat','$tmp12')");
            echo $tmp12 . " ";
        }
