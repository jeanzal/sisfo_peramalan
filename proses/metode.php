...
while ($c = mysqli_fetch_array($sql2)) {
    $a = 0.1;
    $sql = mysqli_query($conn, "SELECT SUM(jumlah_keluar) AS lembar, YEAR(tanggal_keluar) as tahun FROM penjualan WHERE 
 barang_id='$c[id_barang]' GROUP BY YEAR(tanggal_keluar) LIMIT 5"); // memanggil jumlah data penjualan berdasarkan bulan dan tahun
